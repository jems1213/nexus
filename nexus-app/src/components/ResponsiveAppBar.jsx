...
function ResponsiveAppBar() {
  ...
  const getIcon = (page) => {
    if (page === 'Products') return faBoxOpen;
    if (page === 'Pricing') return faTags;
    if (page === 'Blog') return faNewspaper;
  };
            ...
            <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
              ...
              <Menu anchorEl={anchorElNav} open={Boolean(anchorElNav)} onClose={handleCloseNavMenu}
                sx={{
                  ...
                  '& .MuiPaper-root': { backgroundColor: colors.primary, color: colors.textLight }
                }}
              >
                {pages.map((page) => (
                  <MenuItem 
                ...
                {!isLoggedIn && (
                  <>
                    <MenuItem onClick={() => { handleLogin(); handleCloseNavMenu(); }}>
                      <Typography><FontAwesomeIcon icon={faSignInAlt} style={{ marginRight: 6 }} /> Login</Typography>
                    </MenuItem>
                    <MenuItem onClick={() => { handleSignup(); handleCloseNavMenu(); }}>
                      <Typography><FontAwesomeIcon icon={faUserPlus} style={{ marginRight: 6 }} /> Sign Up</Typography>
                    </MenuItem>
                  </>
                )}
              </Menu>
            ...
            <Box sx={{ flexGrow: 0, display: 'flex', alignItems: 'center' }}>
              ...
              >
                {settings.map((setting) => (
                  ...
                  >
                    {setting === 'Profile' && (
                      <FontAwesomeIcon icon={faUser} style={{ marginRight: 8, width: 16 }} />
                    )}
                    {setting === 'Dashboard' && (
                      <FontAwesomeIcon icon={faTachometerAlt} style={{ marginRight: 8, width: 16 }} />
                    )}
                    {setting === 'AboutUs' && (
                      <FontAwesomeIcon icon={faInfoCircle} style={{ marginRight: 8, width: 16 }} />
                    )}
                    {setting === 'Logout' && (
                      <FontAwesomeIcon icon={faSignOutAlt} style={{ marginRight: 8, width: 16 }} />
                    )}
                    <Typography textAlign="center">
